...
<div class="flex justify-between items-center mb-6">
    <div>
        <h2 class="text-2xl font-bold text-[#212529]">知識分享維護</h2>
        <p class="text-[#6C757D]">管理知識分享資料，置頂的內容會顯示在前台首頁</p>
    </div>
    <div class="flex items-center space-x-4">
        <div class="text-sm text-[#6C757D]">
            <i class="fas fa-thumbtack mr-1"></i>
            置頂 <span class="font-semibold text-[#212529]"><?= $pinnedCount ?></span> / <?= $maxPinned ?>
        </div>
        <a href="<?= url('/admin/knowledge/create') ?>" class="bg-[#4A90D9] text-white px-4 py-2 rounded-lg hover:bg-[#357ABD] transition">
            <i class="fas fa-plus mr-2"></i>新增知識分享
        </a>
    </div>
</div>
...
<div class="bg-white rounded-lg border border-[#E9ECEF] shadow-sm mb-6 p-4">
    <form method="GET" class="flex flex-wrap gap-4">
        ...
        <div class="w-32">
            <select name="is_pinned" class="w-full px-4 py-2 border border-[#DEE2E6] rounded-lg focus:ring-2 focus:ring-[#4A90D9] focus:border-[#4A90D9] text-[#212529]">
                <option value="">全部置頂</option>
                <option value="1" <?= ($isPinned ?? '') === '1' ? 'selected' : '' ?>>已置頂</option>
                <option value="0" <?= ($isPinned ?? '') === '0' ? 'selected' : '' ?>>未置頂</option>
            </select>
        </div>
        <button type="submit" class="bg-[#6C757D] text-white px-4 py-2 rounded-lg hover:bg-[#5a6268]">
            <i class="fas fa-search mr-2"></i>搜尋
        </button>
        <a href="<?= url('/admin/knowledge') ?>" class="bg-[#F8F9FA] text-[#212529] px-4 py-2 rounded-lg hover:bg-[#E9ECEF] border border-[#DEE2E6]">
    ...
    <table class="min-w-full divide-y divide-[#E9ECEF]">
        <thead class="bg-[#F8F9FA]">
            <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#212529] uppercase w-20">置頂</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#212529] uppercase w-24">封面</th>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#212529] uppercase w-40">建立時間</th>
        ...
        <tbody class="bg-white divide-y divide-[#E9ECEF]">
            <?php if (empty($items)): ?>
            <tr>
                <td colspan="8" class="px-6 py-8 text-center text-[#6C757D]">
            ...
            <tr class="hover:bg-[#F5F5F5]">
                ...
                <td class="px-4 py-3 text-center">
                    <button onclick="togglePinned(<?= $item['id'] ?>)" class="focus:outline-none" title="<?= $item['is_pinned'] ? '取消置頂' : '設為置頂' ?>">
                        <?php if ($item['is_pinned'] == 1): ?>
                        <span class="inline-flex items-center justify-center w-8 h-8 bg-[#FFF3CD] rounded-full text-[#856404] hover:bg-[#ffe4a0] transition">
                            <i class="fas fa-thumbtack"></i>
                        </span>
                        <?php else: ?>
                        <span class="inline-flex items-center justify-center w-8 h-8 bg-[#F8F9FA] rounded-full text-[#ADB5BD] hover:bg-[#E9ECEF] transition">
                            <i class="fas fa-thumbtack"></i>
                        </span>
                        <?php endif; ?>
                    </button>
                ...
                <td class="px-4 py-3">
                    <div class="text-sm font-medium text-[#212529]"><?= htmlspecialchars($item['title']) ?></div>
                ...
                <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                    <a href="<?= url('/admin/knowledge/edit/' . $item['id']) ?>" class="text-[#4A90D9] hover:text-[#357ABD] mr-3" title="編輯">
...
<div class="mt-6 flex justify-between items-center">
    ...
    <nav class="flex space-x-2">
        ...
        <a href="?page=<?= $pagination['current_page'] - 1 ?>&search=<?= urlencode($search ?? '') ?>&status=<?= urlencode($status ?? '') ?>&category=<?= urlencode($category ?? '') ?>&is_pinned=<?= urlencode($isPinned ?? '') ?>" 
        ...
        <a href="?page=<?= $i ?>&search=<?= urlencode($search ?? '') ?>&status=<?= urlencode($status ?? '') ?>&category=<?= urlencode($category ?? '') ?>&is_pinned=<?= urlencode($isPinned ?? '') ?>" 
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6 border border-[#E9ECEF]">
            <div class="text-center">
                ...
                <p class="text-[#6C757D] mb-6">您確定要刪除知識分享 <span id="delete-item-name" class="font-semibold text-[#212529]"></span> 嗎？此操作無法復原。</p>
...
<script>
    ...
    async function toggleStatus(id) {
        try {
            const response = await fetch(`<?= url('/admin/knowledge/toggle-status/') ?>${id}`, {
                method: 'POST',
                headers: {
    ...
    async function togglePinned(id) {
        try {
            const response = await fetch(`<?= url('/admin/knowledge/toggle-pinned/') ?>${id}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': getCsrfToken()
    ...
    async function confirmDelete() {
        ...
        try {
            const response = await fetch(`<?= url('/admin/knowledge/delete/') ?>${deleteItemId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': getCsrfToken()
