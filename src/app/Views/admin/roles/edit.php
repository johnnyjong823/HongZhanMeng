...
<div class="flex justify-between items-center mb-6">
    <div>
        <h2 class="text-2xl font-bold text-[#212529]">編輯角色</h2>
        <p class="text-[#6C757D]">編輯角色：<?= htmlspecialchars($role['role_name'] ?? $role['name'] ?? '') ?></p>
    </div>
...
<div class="bg-white rounded-lg border border-[#E9ECEF] shadow-sm p-6">
    <form id="edit-role-form" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" required
                       value="<?= htmlspecialchars($role['role_name'] ?? $role['name'] ?? '') ?>"
            ...
            <div>
                ...
                <select name="level" required
                    ...
                    <option value="<?= $value ?>" <?= ($role['level'] ?? '') == $value ? 'selected' : '' ?>><?= htmlspecialchars($label) ?></option>
                    <?php endforeach; ?>
                </select>
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full px-4 py-2 border border-[#DEE2E6] rounded-lg focus:ring-2 focus:ring-[#4A90D9] focus:border-[#4A90D9] text-[#212529]"
                      placeholder="角色說明（選填）"><?= htmlspecialchars($role['description'] ?? '') ?></textarea>
        </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="functions[]" value="<?= $function['id'] ?>" 
                               ...
                               <?= in_array($function['id'], $roleFunctions ?? []) ? 'checked' : '' ?>
                    ...
                    <div class="ml-6 space-y-2">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="functions[]" value="<?= $child['id'] ?>" 
                                   id="func_<?= $child['id'] ?>"
                                   <?= in_array($child['id'], $roleFunctions ?? []) ? 'checked' : '' ?>
                                   class="w-4 h-4 text-[#4A90D9] border-[#DEE2E6] rounded focus:ring-[#4A90D9]">
...
<script>
    document.getElementById('edit-role-form').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('<?= url('/admin/roles/update/' . $role['id']) ?>', {
                method: 'POST',
                headers: {
