...
<div class="flex justify-between items-center mb-6">
    <div>
        <h2 class="text-2xl font-bold text-[#212529]">編輯功能</h2>
        <p class="text-[#6C757D]">編輯功能：<?= htmlspecialchars($function['function_name'] ?? $function['name'] ?? '') ?></p>
    </div>
...
<div class="bg-white rounded-lg border border-[#E9ECEF] shadow-sm p-6">
    <form id="edit-function-form" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" required
                       value="<?= htmlspecialchars($function['function_name'] ?? $function['name'] ?? '') ?>"
            ...
            <div>
                ...
                <input type="text" name="code" required
                       value="<?= htmlspecialchars($function['function_code'] ?? $function['code'] ?? '') ?>"
                       class="w-full px-4 py-2 border border-[#DEE2E6] rounded-lg focus:ring-2 focus:ring-[#4A90D9] focus:border-[#4A90D9] text-[#212529]"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="parent_id"
                    ...
                    <option value="<?= $parent['id'] ?>" <?= ($function['parent_id'] ?? '') == $parent['id'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars($parent['function_name'] ?? $parent['name'] ?? '') ?>
                    </option>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </select>
            ...
            <div>
                ...
                <input type="text" name="url"
                       value="<?= htmlspecialchars($function['url'] ?? '') ?>"
                       class="w-full px-4 py-2 border border-[#DEE2E6] rounded-lg focus:ring-2 focus:ring-[#4A90D9] focus:border-[#4A90D9] text-[#212529]"
                       placeholder="例如：/admin/users">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="icon"
                       value="<?= htmlspecialchars($function['icon'] ?? '') ?>"
                       class="w-full px-4 py-2 border border-[#DEE2E6] rounded-lg focus:ring-2 focus:ring-[#4A90D9] focus:border-[#4A90D9] text-[#212529]"
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="<?= $function['sort_order'] ?? 0 ?>"
                       class="w-full px-4 py-2 border border-[#DEE2E6] rounded-lg focus:ring-2 focus:ring-[#4A90D9] focus:border-[#4A90D9] text-[#212529]"
                       placeholder="0">
        ...
        <div>
            <label class="flex items-center">
                <input type="checkbox" name="is_menu" value="1" <?= ($function['is_menu'] ?? 0) == 1 ? 'checked' : '' ?>
...
<script>
    document.getElementById('edit-function-form').addEventListener('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            const response = await fetch('<?= url('/admin/functions/update/' . $function['id']) ?>', {
                method: 'POST',
                headers: {
