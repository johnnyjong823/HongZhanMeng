<div class="min-h-screen flex items-center justify-center bg-[#F8F9FA] py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-xl border border-[#E9ECEF] shadow-sm">
        <div class="text-center">
            <h1 class="text-3xl font-bold text-[#212529]">忘記密碼</h1>
            <p class="mt-2 text-sm text-[#6C757D]">請輸入您的 Email，我們將發送密碼重設連結</p>
        </div>
        ...
        <form id="forgot-form" class="mt-8 space-y-6">
            ...
            <div>
                <label for="email" class="block text-sm font-medium text-[#212529]">Email</label>
...
<script>
    document.getElementById('forgot-form').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('<?= url('/account/do-forgot-password') ?>', {
                method: 'POST',
                headers: {
